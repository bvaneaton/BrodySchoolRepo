...
public enum ASMOpcode {
	...
	// File input/output
	FileInput,		// Reads an integer array from a file.  Takes two args from the top of the stack.  
					// The top of stack is a string (pointer) filename.  The second arg is a place
					// in memory to put the array.  Arrays are stored in memory as the following
					// in sequence: first, an int that is the number of dimensions.  Next, for each dimension,
					// two integers (a lowest index on the dimension, and the number of elements in that dimension)
					// Finally, the data itself is stored, with the convention that later dimensions vary more.
					// (e.g. from one element to the next in memory, the index of the last dimension will change)
					// When FileInput finishes, the number of bytes read (and written to memory) is put on the
					// top of the stack.

	FileOutput,		// The opposite of FileInput...writes an integer array to a file.  Takes two args from
					// the top of the stack: filename and pointer to the array.  Does not push the number of
					// bytes written.
	...
	// but their definition does not need to precede their usage.
	...
	private boolean labelJumpOrCall() {
		return this == Label || this == Call || (this.name().indexOf("Jump") == 0);
	}
}
